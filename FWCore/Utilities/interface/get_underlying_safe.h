...
namespace edm {
  ...
  template <typename T>
  std::shared_ptr<T[]>& get_underlying_safe(propagate_const_array<std::shared_ptr<T[]>>& iP) {
    return get_underlying(iP);
  }
  template <typename T>
  std::shared_ptr<T const []> get_underlying_safe(propagate_const_array<std::shared_ptr<T[]>> const& iP) {
    std::shared_ptr<T const[]> copy = get_underlying(iP);
    return copy;
  }
  ...
  template <typename T>
  T* get_underlying_safe(propagate_const_array<T[]>& iP) {
    return get_underlying(iP);
  }
  template <typename T>
  T const* get_underlying_safe(propagate_const_array<T[]> const& iP) {
    T const* copy = get_underlying(iP);
    return copy;
  }
  ...
  template <typename T>
  std::unique_ptr<T[]>& get_underlying_safe(propagate_const_array<std::unique_ptr<T[]>>& iP) {
    return get_underlying(iP);
  }
  // the template below will deliberately not compile.
  template <typename T>
  std::unique_ptr<T const []> get_underlying_safe(propagate_const_array<std::unique_ptr<T[]>> const& iP) {
    std::unique_ptr<T const[]> copy = get_underlying(iP);
    return copy;
  }
